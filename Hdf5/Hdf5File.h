 ...
 *            18 February  2020, 16:39 (revised)
 *
 * @section   HDF5File HDF5 File Structure
 *
 * The CUDA/C++ code has been designed as a standalone application supporting both 2D and 3D simulations which are not
 * dependent on MATLAB libraries or a MEX interface. This is of particular importance when using servers and
 * supercomputers without MATLAB support. For this reason, simulation data must be transferred between the CUDA/C++
 * code and MATLAB using external input and output files. These files are stored using the
 * [Hierarchical Data Format HDF5] (http://www.hdfgroup.org/HDF5/). This is a data model, library, and file format for
 * storing and managing data. It supports a variety of datatypes, and is designed for flexible and efficient I/O and
 * for high volume and complex data. The HDF5 technology suite includes tools and applications for managing,
 ...
 *
 *
 * kspaceFirstOrder-CUDA v1.3 uses a new file format of version 1.2, which adds support for an axisymmetric coordinate
 * system although not supported in the CUDA code at the moment) and removes the definition of derivative and shift
 * operators, as well as the PML. These variables are instead generated in the preprocessing phase and make the file
 * structure simpler. The code is happy to work with all previous file versions (1.0 and 1.1), however, some features
 * will not be supported. Namely, the cuboid sensor mask, and <tt>u_non_staggered_raw</tt> are not supported in version
 * 1.0. When running from the C++/CUDA code using MATLAB k-Wave Toolbox v1.3, the files will always be written in file
 * format version 1.2. The output file is always written in version 1.2.
 *
 *
 ...
 *
 * The HDF5 input file for the CUDA/C++ simulation code contains a file header with a brief description of the
 * simulation stored in string attributes, and the root group <tt>'/'</tt> which stores all the simulation properties in
 * the form of 3D datasets irrespective of whether the simulation is in 2D or 3D. In the case of 2D simulation, Nz
 * equals to 1. A complete list of input datasets is given below.
 *
